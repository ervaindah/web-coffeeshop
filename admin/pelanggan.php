<?php  
    include 'index.php';
?>

        <div id="layoutSidenav">
           <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h3 class="mt-4">Data Pelanggan</h3>
                        
                    </div>
                </main>

                 <!-- isi inti -->
                <div class="card mt-3 mb-4">
                    <div class="card-body">
                        <table id="datatablesSimple"> <!-- table bawaan jvscript "datatableSimple" -->
                            <thead>
                                <tr class="table-secondary" >
                                    <th>No</th>
                                    <th>Nama</th>
                                    <th>Telepon</th>
                                    <th>Aksi</th>
                                    
                                </tr>
                            </thead> 
                
                                <tbody>
                                    <?php
                                        //menampilkan kategori 
                                        $no = 1;
                                        $ambil = mysqli_query($conn, "SELECT * FROM pelanggan"); ?>

                                            
                                            <?php while($row = mysqli_fetch_array($ambil)){ ?>
                                            <!-- tr akan dilooping -->
                                                    <tr>
                                                        <td><?php echo $no++ ?></td>
                                                          <!-- menampilkan nama data -->
                                                        <td><?php echo $row['nama_pelanggan'] ?></td>
                                                        <td><?php echo $row['telepon_pelanggan'] ?></td>
                                                        <!--end-->
                                                          <td>
                                                            <!--  ketika msuk halaman edit/hpus maka id akan terbawa -->
                                                            <a href="proses-hapus.php? id=<?php echo $row['id_pelanggan'] ?>" class="btn btn-danger btn" onclick="return confirm('Anda yakin ingin dihapus?')">Hapus</a>
                                                          </td>
                                                    </tr>
                                                <?php } ?>
                                </tbody>
                            </table>
                        </div>
                </div>

                    <div class="container">
                        <a href="tambah-pelanggan.php" class="btn btn-primary">Tambah Data</a>
                    </div>

                    <?php
                        include "footer.php";
                    ?>

            </div>
        </div>

        